...
import { Wallet, Plus, TrendingUp, AlertTriangle, CheckCircle, Trash2, ArrowLeft } from 'lucide-react';
...
export default function PersonalBudget() {
  ...
  useEffect(() => {
    if (!user) return;
    const key = `personal_budget_${user.id}`;
    const saved = localStorage.getItem(key);
    if (saved) {
      const data = JSON.parse(saved);
      setBudget(data.budget || 0);
      setExpenses(data.expenses || []);
      setHasBudget(!!data.budget);
    }
  }, [user]);
  ...
  useEffect(() => {
    if (!user) return;
    const key = `personal_budget_${user.id}`;
    localStorage.setItem(key, JSON.stringify({ budget, expenses }));
  ...
  const handleSetBudget = () => {
    ...
    setHasBudget(true);
    setBudgetInput('');
    toast.success(`Budget set to ₹${val.toLocaleString('en-IN')}`);
  };
  ...
  const handleAddExpense = () => {
    ...
    if (totalSpent + amtNum > budget && budget > 0) {
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-card rounded-2xl p-5 shadow-soft border border-border/50">
            {!hasBudget ? (
              <div className="text-center">
                ...
                <h3 className="font-bold text-foreground mb-2">Set Your Monthly Budget</h3>
                <p className="text-sm text-muted-foreground mb-4">Enter how much you plan to spend this month</p>
                <div className="flex gap-2 max-w-xs mx-auto">
                  <div className="relative flex-1">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground font-bold">₹</span>
                    <Input type="number" value={budgetInput} onChange={e => setBudgetInput(e.target.value)} placeholder="10,000" className="pl-8 h-12 rounded-xl" />
                  </div>
                  <Button onClick={handleSetBudget} variant="gradient" className="h-12 rounded-xl px-6">Set</Button>
                </div>
              </div>
            ) : (
              <>
                <div className="flex items-center justify-between mb-3">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-foreground">₹{budget.toLocaleString('en-IN')}</p>
                  </div>
                ...
                <button onClick={() => { setHasBudget(false); setBudget(0); setExpenses([]); }} className="text-xs text-muted-foreground hover:text-destructive mt-3 transition-colors">
                  Reset Budget
                </button>
              </>
            )}
